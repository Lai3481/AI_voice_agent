...
subsidy_docs = SimpleDirectoryReader(input_files=[r"C:\Degree\Y3S1\mathematical_software\assignment\Malaysia 2023-01-01 to 2023-12-31.txt"]).load_data()
...
# as_query_engine(similarity_top_k=6) 方法将向量索引转换为一个查询引擎。
# similarity_top_k=6 参数指定在查询时返回的最相似的文档数量。
# 在这个例子中，查询引擎将返回与输入查询最相似的6个文档。
